			      ...
			      const char *name)
{
	...
	for (i = 0; i < sizeof(ppc64_stub_insns)/sizeof(u32); i++) {
		patch_instruction(&entry->jump[i], ppc_inst(ppc64_stub_insns[i]));
	}
	...
	patch_instruction(&entry->jump[0], ppc_inst(entry->jump[0] | PPC_HA(reladdr)));
	patch_instruction(&entry->jump[1], ppc_inst(entry->jump[1] | PPC_LO(reladdr)));
